...
from posteriors.sgmcmc import baoa_nht

from tests.scenarios import batch_normal_log_prob


def test_baoa_nht_nht():
    ...
    sampler = baoa_nht.build(batch_normal_log_prob_spec, lr=lr, alpha=alpha)

    # Test inplace = False
    baoa_nht_state = sampler.init(params)
    log_posts = []
    all_params = tree_map(lambda x: x.unsqueeze(0), params)

    for _ in range(n_steps):
        baoa_nht_state = sampler.update(baoa_nht_state, batch, inplace=False)

        all_params = tree_map(
            lambda x, y: torch.cat((x, y.unsqueeze(0))),
            all_params,
            baoa_nht_state.params,
        )

        log_posts.append(baoa_nht_state.log_posterior.item())
    ...
    # Test inplace = True
    baoa_nht_state = sampler.init(params, momenta=0.0)
    log_posts = []
    all_params = tree_map(lambda x: x.unsqueeze(0), params)

    for _ in range(n_steps):
        baoa_nht_state = sampler.update(baoa_nht_state, batch, inplace=True)
