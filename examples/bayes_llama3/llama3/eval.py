...
from llama3.utils.load_utils import load_ensemble, save_config
from llama3.utils.prompting import llama_chat_prompt, ONE_SHOT
...
class Experiment:
    def __init__(self, config: FrozenConfigDict):
        ...
        assert config["bayes_config"]["n_ensemble"] == len(config["checkpoint_paths"])
        parameters = load_ensemble(config["checkpoint_paths"])

        self.model = BayesLlamaForCausalLM.from_pretrained(
            config["pretrained_model_name_or_path"],
            bayes_config=dict(config["bayes_config"]),
        )
    ...
    def run_experiment(self, dataset_path: str):
        ...
        for _, sample in tqdm(enumerate(dataset)):
            ...
            inputs = [
                self.prepare_prompt(prompt=PROMPT, question=question, document=context)
